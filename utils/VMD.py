def VMD(signal, alpha, tau, K, DC, init, tol, plot=False):
    # ---------------------
    ...
    u_hat_plus = np.zeros((N, len(freqs), K), dtype=np.float64)
    # print('u_hat_plus')
    ...
    lamda_hat = np.zeros((N, len(freqs)), dtype=np.float64)
    ...
    u_hat = np.zeros((T, K), dtype=np.float64)
    ...
    # print(u_hat.shape)
    # recompute spectrum
    u_hat = np.zeros((T // 2, K), dtype=np.float64)
    ...
    # print(omega)
    if plot:
        num = str(K + 1)+ str(1)+ str(0)
        plt.subplot(int(num))
        plt.plot(signal)
        for i in range(1, K + 1):
            num = str(K + 1) + str(1) + str(i)
            plt.subplot(int(num))
            plt.plot(u[i - 1, :])
        plt.show()
    u = u.T
